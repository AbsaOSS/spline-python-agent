...
class ObjectFactory:
    ...
    def instantiate(self, typ: Type[T]) -> T:
        ...
        prefix: str = f'spline.{camel_to_snake(typ.__name__)}'
        object_name: str = self.__config[f'{prefix}.type']
        full_classname: str = self.__config[f'{prefix}.{object_name}.class_name']

        logger.debug(f'found candidate: {full_classname}')
